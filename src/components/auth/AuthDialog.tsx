...
function AuthDialog({ message, onClose, onComplete }: Props) {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-50 px-2 sm:px-0 flex items-center justify-center bg-bg/15 backdrop-blur-m"
    >
      <div className="relative w-full max-w-[640px] px-6 py-8 rounded-2xl flex flex-col items-center justify-center gap-4">
        ...
        {/* overlay */}
        <div className="auth-popup absolute -z-10 inset-0 blur-3xl backdrop-blur-3xl rounded-2xl drop-shadow-2xl overflow-visible" />
      </div>
    </div>
  );
}
